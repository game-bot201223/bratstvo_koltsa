...
begin
  ...
  -- If reward is already claimed, do not apply further damage (prevents resurrect/desync).
  update public.player_boss_fights
  set
    hp = case
      when reward_claimed then hp
      else greatest(0, hp - greatest(0, coalesce(p_dmg, 0)))
    end,
    max_hp = greatest(max_hp, p_max_hp),
    expires_at = coalesce(p_expires_at, expires_at),
    updated_at = now()
  where owner_tg_id = trim(p_owner_tg_id)
    and boss_id = p_boss_id
  returning owner_tg_id, boss_id, hp, max_hp, expires_at, reward_claimed
...
begin
  ...
  -- Already claimed: report as not-claimable (prevents double rewards on client).
  if v_claimed then
