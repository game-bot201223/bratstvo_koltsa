...
async function postgrestGetClan(projectUrl: string, serviceKey: string, clanId: string): Promise<any | null> {
  const url = projectUrl.replace(/\/$/, "") + `/rest/v1/clans?id=eq.${encodeURIComponent(clanId)}&select=id,owner_tg_id&limit=1`
  const resp = await fetch(url, {
    method: "GET",
...
Deno.serve(async (req: Request) => {
  ...
  // Soft rate limit: avoid double delete clicks.
  try {
    const tgId = String(verified.user.id)
    const rlKey = `delete_clan:${tgId}`
    const rl = await postgrestRateLimitAllow(projectUrl, serviceKey, rlKey, 1500)
    if (rl.ok && !rl.allowed) {
  ...
  if (String(existing.owner_tg_id || "") !== tgId) {
  ...
  const resp = await postgrestDeleteClan(projectUrl, serviceKey, clanId)
  if (!resp.ok) {
