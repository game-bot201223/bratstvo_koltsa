...
async function postgrestUpsertClan(projectUrl: string, serviceKey: string, payload: Record<string, unknown>): Promise<Response> {
  const url = projectUrl.replace(/\/$/, "") + "/rest/v1/clans?on_conflict=id"
...
Deno.serve(async (req: Request) => {
  ...
  // Soft rate limit: clan upserts are mutations; avoid double submits.
  try {
    const tgId = String(verified.user.id)
    const rlKey = `upsert_clan:${tgId}`
  ...
  const tgId = String(verified.user.id)
  const clanId = sanitizeClanId(body?.id)
  ...
  if (!data || typeof data !== "object") {
    return new Response(JSON.stringify({ ok: false, error: "bad_data" }), {
      status: 400,
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    })
  }

  const existing = await postgrestGetClan(projectUrl, serviceKey, clanId)
  if (existing && String(existing.owner_tg_id || "") !== tgId) {
  ...
  const resp = await postgrestUpsertClan(projectUrl, serviceKey, payload)
  if (!resp.ok) {
    const text = await resp.text().catch(() => "")
    try {
      if (/duplicate key value/i.test(text) && /(clans_name_lower_uniq|clan name|name_lower)/i.test(text)) {
        return new Response(JSON.stringify({ ok: false, error: "clan_name_taken" }), {
          status: 409,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        })
      }
