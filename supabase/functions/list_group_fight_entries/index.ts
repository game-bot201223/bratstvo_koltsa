...
async function postgrestListEntries(projectUrl: string, serviceKey: string, startTsIso: string): Promise<Response> {
  const url = projectUrl.replace(/\/$/, "") +
    `/rest/v1/group_fight_entries?select=tg_id,name,photo_url,stats_sum&start_ts=eq.${encodeURIComponent(startTsIso)}&order=stats_sum.desc&limit=5000`
...
Deno.serve(async (req: Request) => {
  ...
  const resp = await postgrestListEntries(projectUrl, serviceKey, startTsIso)
  if (!resp.ok) {
  ...
  return new Response(JSON.stringify({ ok: true, entries: rows, start_ts: startTsIso }), {
    headers: { ...corsHeaders, "Content-Type": "application/json" },
  })
})
