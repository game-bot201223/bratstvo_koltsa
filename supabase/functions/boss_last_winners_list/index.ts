...
async function postgrestListWinners(projectUrl: string, serviceKey: string): Promise<any[]> {
  const url = projectUrl.replace(/\/$/, "") +
    "/rest/v1/boss_last_winners?select=boss_id,tg_id,name,photo_url,updated_at&order=boss_id.asc&limit=50"
