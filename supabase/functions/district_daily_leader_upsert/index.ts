...
async function postgrestUpsert(projectUrl: string, serviceKey: string, row: any): Promise<boolean> {
  const url = projectUrl.replace(/\/$/, "") + "/rest/v1/district_daily_leaders"
  const resp = await fetch(url, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
...
Deno.serve(async (req: Request) => {
  ...
  try {
    const rlKey = `district_daily_leader_upsert:${String(verified.user.id)}`
  ...
  const ok = await postgrestUpsert(projectUrl, serviceKey, row)
  if (!ok) {
