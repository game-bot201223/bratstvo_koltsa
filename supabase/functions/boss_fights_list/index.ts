...
async function postgrestListFights(projectUrl: string, serviceKey: string, ownerTgId: string): Promise<any[]> {
  const url = projectUrl.replace(/\/$/, "") +
    `/rest/v1/player_boss_fights?owner_tg_id=eq.${encodeURIComponent(ownerTgId)}&select=boss_id,hp,max_hp,expires_at,reward_claimed,updated_at&order=boss_id.asc&limit=200`
...
Deno.serve(async (req: Request) => {
  ...
  const fights = await postgrestListFights(projectUrl, serviceKey, ownerTgId)

  return new Response(JSON.stringify({ ok: true, fights }), {
    headers: { ...corsHeaders, "Content-Type": "application/json" },
  })
})
