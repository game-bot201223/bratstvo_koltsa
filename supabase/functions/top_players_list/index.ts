...
async function postgrestListTopPlayers(
  projectUrl: string,
  serviceKey: string,
  kind: string,
  limit: number,
  ...
  let order = "level.desc"
  if (k === "LVL") order = "level.desc"
  else if (k === "WEALTH") order = "stats_sum.desc"
  else if (k === "BOSS") order = "boss_wins.desc"

  const url = projectUrl.replace(/\/$/, "") +
    `/rest/v1/players?select=tg_id,name,photo_url,level,stats_sum,boss_wins&order=${encodeURIComponent(order)}&limit=${encodeURIComponent(String(limit))}`
...
Deno.serve(async (req: Request) => {
  ...
  const kind = String(body?.kind || "LVL").trim().toUpperCase()
  const limit = Math.min(100, Math.max(1, safeInt(body?.limit, 100)))
  const players = await postgrestListTopPlayers(projectUrl, serviceKey, kind, limit)

  return new Response(JSON.stringify({ ok: true, kind, players }), {
    headers: { ...corsHeaders, "Content-Type": "application/json" },
  })
})
