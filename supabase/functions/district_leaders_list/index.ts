...
async function postgrestListLeaders(projectUrl: string, serviceKey: string): Promise<any[]> {
  const url = projectUrl.replace(/\/$/, "") +
    "/rest/v1/district_leaders?select=district_key,tg_id,name,fear,photo_url,updated_at"
...
Deno.serve(async (req: Request) => {
  ...
  const rows = await postgrestListLeaders(projectUrl, serviceKey)
