...
Deno.serve(async (req: Request) => {
  ...
  // Soft rate limit: avoid repeated clear clicks.
  try {
    const tgId = String(verified.user.id)
    const rlKey = `clan_chat_clear:${tgId}`
  ...
  const meN = normName(memberName)
  const leaderN = normName((clanObj as any).leader)
  const deputyN = normName((clanObj as any).deputy)
  if (!(meN && (meN === leaderN || meN === deputyN))) {
